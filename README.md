...
| `advanced.dry_run` | `false` | 只记日志不真的戳，调参时用 |
...
| 层次 | 文件 | 依赖 | 验证内容 |
| --- | --- | --- | --- |
| 离线逻辑测试 | `tests/poke/run_tests.py` | 无（用 `tests/stubs` 里的桩模块） | 三个场景的概率门槛与冷却、一轮对话只戳一次、poke 通知与空消息链的跳过、目标解析（QQ 号 / 昵称 / 留空）、不戳自己、白黑名单、每小时上限、最小间隔、管理员跳过、dry-run、发送降级与 packetBackend 报错翻译 |
